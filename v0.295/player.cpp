...
void player::render()
{
	...
	if (_playerState == PLAYER_DASH)
	{
		...
		else
		{
			...
			if (Efect1 >= 20)
			{
				playerDashEfectImg1->alphaFrameRender(getMemDC(), rcPlayer.left - 30, rcPlayer.top + 10, Efect2);
			}
			if (Efect2 >= 20)
			{
			...
			if (Efect3 >= 20)
			{
				playerDashEfectImg1->alphaFrameRender(getMemDC(), rcPlayer.left - 50, rcPlayer.top + 10, Efect2);
			}
			if (Efect4 >= 20)
			{
