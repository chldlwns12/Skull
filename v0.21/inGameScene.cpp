...
HRESULT inGameScene::init()
{
	...
	//백그라운드 렉트 - 이준
	rcIngameBg = RectMake(0, -96, 1940, 810);
	...
	//미니맵
	rcIngameMini = RectMake(WINSIZEX - 194, 0, 194, 81);
	rcIngameMcamera = RectMake(WINSIZEX - 194, 10, 128, 72);
...
void inGameScene::update()
{
	...
	for (int i = 0; i < TILEY; i++)
	{
		for (int j = 0; j < TILEX; j++)
		{
			if (!(sIngameTile[i * TILEX + j].isTouch)) continue;
			
			if (sIngameTile[i * TILEX + j].rc.top < ptPlayer->getRcPlayer().bottom)
			{
				ptPlayer->setPlayerY(sIngameTile[i * TILEX + j].rc.top - 32.1f);
