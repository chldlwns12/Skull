...
HRESULT player::init()
{
	...
	DATA->setPlayerX(pixelX);
	DATA->setPlayerY(pixelY);
...
void player::update()
{
	...
	DATA->setPlayerRc(RectMakeCenter(DATA->getPlayer().x, DATA->getPlayer().y, 80, 64));
	...
	DATA->setPixelCameraX(pixelX);
	DATA->setPixelCameraY(pixelY);
	DATA->setPixelCameraRc(RectMakeCenter(DATA->getPixelCamera().y, DATA->getPixelCamera().y, WINSIZEX, 200));

	if (DATA->getStage() == STAGE_1 && isMove == false)
	{
		playerX = 50.f;
		DATA->setPlayerX(50.f);
		playerY = 558.f;
		DATA->setPlayerY(558.f);
		isMove = true;
	}
	if (DATA->getStage() == START && isMove == true)
	{
		playerX = 1200;
		DATA->setPlayerX(1200);
		isMove = false;
	}
	if (DATA->getStage() == STAGE_2 && isMove == true)
	{
		playerX = 100.f;
		DATA->setPlayerX(100.f);
		playerY = 100;
		DATA->setPlayerY(100.f);
	}
	...
	if (INPUT->GetKey(VK_RIGHT))
	{
		...
		if (DATA->getPlayerRc().right < DATA->getPixelGroundRc().right)
		{
			pixelX += playerSpeed;
			DATA->setPlayerX(pixelX);
		}
	...
	if (INPUT->GetKey(VK_LEFT))
	{
		...
		//if (DATA->getPlayerRc().left > DATA->getPixelGroundRc().left)
		//{
			pixelX -= playerSpeed;
			DATA->setPlayerX(pixelX);
		//}
...
void player::render()
{
	Rectangle(getMemDC(), DATA->getPixelCamera().rcCamera);
...
void player::PixelCollision()
{
	...
	/*이부분이 픽셀충돌의 핵심*/
	if (DATA->getStage() == START || DATA->getStage() == STAGE_1_2 || DATA->getStage() == STAGE_2)
	{
