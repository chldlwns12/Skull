...
HRESULT player::init()
{
	...
	dashGroundEfect = IMAGEMANAGER->findImage("대쉬이펙트");
	jumpEfect = IMAGEMANAGER->findImage("점프이펙트");
...
void player::update()
{
	...
	{
		rcSkill = RectMakeCenter(playerX, playerY, 0, 0);
	}
...
void player::render()
{
	...
	if (DATA->getPlayerRuin() == SAMURAI)
	{
		...
		if (_playerState == PLAYER_ATT)
		{
			if (isLeft)
			{
				samuraiAttImg1->frameRender(getMemDC(), rcPlayer.left - 15, rcPlayer.top - 15);
			}
			else
			{
		...
		if (_playerState == PLAYER_ATT2)
		{
			if (isLeft)
			{
				samuraiAttImg2->frameRender(getMemDC(), rcPlayer.left - 35, rcPlayer.top - 25);
			}
			else
			{
		...
		if (_playerState == PLAYER_ATT3)
		{
			if (isLeft)
			{
				samuraiAttImg3->frameRender(getMemDC(), rcPlayer.left - 40, rcPlayer.top - 20);
			}
			else
			{
	...
	if (isJump)
	{
		jumpEfect->frameRender(getMemDC(), rcPlayer.left, rcPlayer.bottom + 15);
	}
	...
	sprintf(str3, "pixelX : %f", DATA->getPixelGround().x);
	TextOut(getMemDC(), 10, 130, str3, strlen(str3));
	sprintf(str6, "pixelY : %f", DATA->getPixelGround().y);
	...
	sprintf(str7, "STATE : %d", _playerState);
...
void player::skulAnimation()
{
	...
	if (isJump)
	{
		...
		if (animationCount % 4 == 0)
		{
			...
			jumpEfect->setFrameX(animationIndex);
		}
...
void player::PixelCollision()
{
	...
	else if (DATA->getStage() == STAGE_2 || DATA->getStage() == STAGE_3 || DATA->getStage() == BOSS_STAGE)
	{
...
void player::playerMove()
{
	...
	if (INPUT->GetKey(VK_RIGHT))
	{
		...
		if (DATA->getPlayerCamera().isRight == true && DATA->getStage() == BOSS_STAGE)
		{
			playerX = WINSIZEX / 2;
		}
	...
	if (INPUT->GetKey(VK_LEFT))
	{
		...
		if (DATA->getPlayerCamera().isLeft == true && DATA->getStage() == BOSS_STAGE)
		{
			playerX = WINSIZEX / 2;
		}
		//isMoving = true;
	...
	if (INPUT->GetKeyDown('X'))
	{
		...
		if (DATA->getPlayerRuin() == SKUL)
		{
			if (isLeft)
			{
				rcATK = RectMake(playerX - 70, playerY - 20, 80, 45);
			}
			else
			{
				rcATK = RectMake(playerX, playerY - 20, 80, 45);
			}
			isATK = true;
		...
		else if (DATA->getPlayerRuin() == SOLDIER)
		{
			if (isLeft)
			{
				rcATK = RectMakeCenter(playerX - 100, playerY - 10, 80, 45);
			}
			else
			{
				rcATK = RectMakeCenter(playerX + 20, playerY - 10, 80, 45);
			}
			isATK = true;
		...
		else if (DATA->getPlayerRuin() == SAMURAI)
		{
			if (isLeft)
			{
				rcATK = RectMakeCenter(playerX - 90, playerY, 120, 45);
			}
			else
			{
				rcATK = RectMakeCenter(playerX + 30, playerY, 120, 45);
			}
			isATK = true;
	...
	{
		rcSkill = RectMakeCenter(playerX, playerY, 50, 50);
		if (isLeft)
		{
			if (animationIndex < 5)
			{
...
void player::setPlayer()
{
	...
	if (DATA->getStage() == BOSS_STAGE && isMove == true)
	{
		playerX = WINSIZEX / 2;
		playerY = 50.f;
		isMove = false;
...
void player::EnemyPlayerATK()
{
	RECT rcTemp12;
	for (int i = 0; i < ptPlayerEnemy->getvEnemy().size(); i++)
	{
		if (IntersectRect(&rcTemp12, &rcPlayer, &ptPlayerEnemy->getvEnemy()[i].rcAtt) && ptPlayerEnemy->getvEnemy()[i].isAttack == true)
		{
			playerCurrentHp -= ptPlayerEnemy->getvEnemy()[i].att;
		}
	}
}

void player::BossPlayerATK()
{
	RECT rcTemp13;
	if ((IntersectRect(&rcTemp13, &rcPlayer, &ptPlayerBoss->getRcLeftHand()) && ptPlayerBoss->getIsAttack() == true) || (IntersectRect(&rcTemp13, &rcPlayer, &ptPlayerBoss->getRcRightHand()) && ptPlayerBoss->getIsAttack() == true))
	{
		playerCurrentHp -= ptPlayerBoss->getBossAtk();
	}
