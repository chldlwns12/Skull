...
{	
	...
	for (int i = 0; i < _vBullet.size(); i++)
	{
		...
		if (_isFrameImg)
		{
			_vBullet[i].rc = RectMakeCenter(DATA->getPixelGround().x + _vBullet[i].x, _vBullet[i].y,
...
void Skul::render()
{
	if (bulletImgState == RENDER)//일반 이미지
	{
		for (int i = 0; i < _vBullet.size(); i++)
		{
			_vBullet[i].bulletImage->render(getMemDC(), _vBullet[i].rc.left, _vBullet[i].rc.top);
	...
	else if(bulletImgState == FRAMERENDER)//프레임 이미지
	{
		for (int i = 0; i < _vBullet.size(); i++)
		{
			_vBullet[i].bulletImage->frameRender(getMemDC(), _vBullet[i].rc.left, _vBullet[i].rc.top);
	...
	else if (bulletImgState == ROTATERENDER)//로테이트 이미지
	{
		for (int i = 0; i < _vBullet.size(); i++)
		{
