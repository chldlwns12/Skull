...
void map3::update()
{
	...
	if (DATA->getPlayer().isChange && DATA->getPlayerRuin() == SAMURAI)
	{
		if (DATA->getPlayer().isLeft == false)
		{
			if (DATA->getPlayerCamera().rcCamera.right >= WINSIZEX)
	...
	if (IntersectRect(&rcTemp2, &rcItem, &DATA->getPlayerRc()) && isLook == true)
	{
		...
		if (INPUT->GetKeyDown('F'))
		{
			if (isNomal)
			{
				...
				if (DATA->getHaveRuin()[1] == NON)
				{
					//PostQuitMessage(0);
					DATA->setHaveRuin(SOLDIER, 1);
				}
...
void map3::render()
{
	if (DATA->getStage() == STAGE_3)
	{
		...
		if (ruinIndex > 7 && DATA->getPlayerRuin() == SKUL && isHave == false)
		{
			if (isNomal)
			{
