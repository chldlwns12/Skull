...
void inGameScene::update()
{
	...
	{
		if (ptPlayer->getRcPlayer().right >= rcIngameBg.right)
		{
			ptPlayer->setPlayerX(rcIngameBg.right - 40);
		}

		if (ptPlayer->getRcPlayerCamera().right >= WINSIZEX && ptPlayer->getRcPlayerCamera().right < rcIngameBg.right)
		{
	...
	{
		if (ptPlayer->getRcPlayer().left <= rcIngameBg.left)
		{
			ptPlayer->setPlayerX(rcIngameBg.left + 40);
		}

		if (ptPlayer->getRcPlayerCamera().left <= 0 && ptPlayer->getRcPlayerCamera().left > rcIngameBg.left)
		{
			if (isCollision == false)
			{
				rcIngameBg.left += 10;
